...
export default function Lorebook({ lorebook, lorebookDraft, editingLore, setEditingLore, TYPE_COLORS, LORE_TYPES, setLorebookDraft, addLorebookEntry, deleteLorebookEntry, toggleLorePin, onOpenEdges, loreEdgePanel, setLoreEdgePanel, entities, graphAPI }) {
  const [panelOpen, setPanelOpen] = useState(() => !!editingLore || !!loreEdgePanel);
  ...
  function openNewEntry() {
    ...
    setLoreEdgePanel(null);
    setPanelOpen(true);
  }

  function openEditEntry(e) {
    setEditingLore(e.id);
    setLorebookDraft({ title: e.title, tags: e.tags ?? "", content: e.content, type: e.type ?? "character" });
    setLoreEdgePanel(null);
    setPanelOpen(true);
  }

  function openEdges(e) {
    onOpenEdges(e);
    setPanelOpen(true);
  }

  return (
    <div style={{ position: "relative", flex: 1, display: "flex", overflow: "hidden", width: "100%" }}>
      <div style={{flex: 1, overflowY: "auto", width: "100%"}}>
      <div style={{ flex: 1, padding: 16, display: "flex", flexDirection: "column", gap: 10, margin: "0 auto", maxWidth: 800, width: "100%" }}>
        {lorebook.length === 0
            ...
            <Card key={e.id} style={{ borderLeft: e.pinned ? "2px solid var(--color-text-info)" : undefined }}>
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
                  <button onClick={() => openEditEntry(e)} style={{ background: "transparent", border: "none", cursor: "pointer", color: "var(--color-text-tertiary)", fontSize: 14 }}><Pencil size={14} /></button>
                  <button onClick={() => openEdges(e)} style={{ background: "transparent", border: "none", cursor: "pointer", color: "var(--color-text-tertiary)", fontSize: 14 }} title="Manage links"><Link size={14} /></button>
                  <button onClick={() => toggleLorePin(e.id, !e.pinned)} style={{ background: "transparent", border: "none", cursor: "pointer", color: e.pinned ? "var(--color-text-info)" : "var(--color-text-tertiary)", fontSize: 14 }} title={e.pinned ? "Unpin" : "Pin — always inject"}>{e.pinned ? <PinOff size={14} /> : <Pin size={14} />}</button>
                  <button onClick={() => deleteLorebookEntry(e.id)} style={{ background: "transparent", border: "none", cursor: "pointer", color: "var(--color-text-tertiary)", fontSize: 17 }}><X size={17} /></button>
                </div>
              </div>
            </Card>
          ))
      ...
      {/* FAB — opens a fresh "new entry" form */}
      {!panelOpen && (<button
        onClick={openNewEntry}
        title="New lorebook entry"
        style={{
          position: "absolute", top: 16, right: 10, zIndex: 12,
      ...
      {/* Slide-in panel — entry form or edge manager */}
      <div
        style={{
          position: "absolute", top: 0, bottom: 0, right: 0, zIndex: 11,
        ...
        {!loreEdgePanel ? (
          <>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <button onClick={() => setPanelOpen(false)} style={{ background: "transparent", border: "none", cursor: "pointer", color: "var(--color-text-tertiary)" }}><X size={16} /></button>
            </div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button
                onClick={() => { if (lorebookDraft.title && lorebookDraft.content) { addLorebookEntry(lorebookDraft); setPanelOpen(false); } }}
                disabled={!lorebookDraft.title || !lorebookDraft.content}
                style={{ ...inputStyle, flex: 1, cursor: "pointer", textAlign: "center", opacity: (!lorebookDraft.title || !lorebookDraft.content) ? 0.4 : 1 }}
              >{editingLore ? "Update" : "Add entry"}</button>
              {editingLore && <button onClick={() => { setEditingLore(null); setLorebookDraft({ title: "", tags: "", content: "", type: "character" }); setPanelOpen(false); }} style={{ ...inputStyle, cursor: "pointer" }}>Cancel</button>}
            </div>
          </>
        ) : (
          <>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <p style={{ margin: 0, fontSize: 13, fontWeight: 500, flex: 1 }}>{loreEdgePanel.entry.title} — links</p>
              <button onClick={() => setPanelOpen(false)} style={{ background: "transparent", border: "none", cursor: "pointer", color: "var(--color-text-tertiary)" }}><X size={16} /></button>
            </div>
            ...
            <div style={{ background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-md)", border: "0.5px solid var(--color-border-tertiary)", padding: "10px 12px", display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              <button onClick={async () => {
                ...
                if (!target) return;
                await graphAPI.createEdge({ source_id: loreEdgePanel.entry.id, target_id: target, relationship: rel, weight: 1.0 });
