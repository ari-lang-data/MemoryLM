...
def get_entities(chat_id: Optional[str] = None, preset_id: Optional[str] = None, type: Optional[str] = None):
    ...
    return [dict(zip(["id","name","type","description","chat_id","preset_id","embedding_id","created_at","metadata"], row)) for row in rows]
...
def get_entity(entity_id: str):
    ...
    row = rows[0]
    return dict(zip(["id","name","type","description","chat_id","preset_id","embedding_id","created_at","metadata"], row))
...
def upsert_character_card(entity_id: str, body: CharacterCardUpdate):
    execute(
        "INSERT OR REPLACE INTO character_cards (id, appearance, behaviour, speech_pattern, background, preset_id, is_active_char, is_user_char) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        [entity_id, body.appearance, body.behaviour, body.speech_pattern, body.background, body.preset_id, body.is_active_char, body.is_user_char]
    )
    return SuccessResponse()
...
def get_characters(preset_id: Optional[str] = None):
    if preset_id:
        rows = execute(
            """SELECT e.*, c.appearance, c.behaviour, c.speech_pattern, c.background, c.is_active_char, c.is_user_char
    ...
    cols = ["id","name","type","description","chat_id","preset_id","embedding_id","created_at","metadata","appearance","behaviour","speech_pattern","background","is_active_char","is_user_char"]
...
def set_active_character(entity_id: str, is_user: bool = False):
    # Deactivate all others for this preset first
...
def set_template_var(body: TemplateVarSet):
    ...
    # Delete existing var with same name + preset
...
def traverse(entity_id: str, depth: int = 1, relationship: Optional[str] = None):
    ...
    def walk(eid: str, current_depth: int):
        ...
        for edge in edges:
            ...
            if target_rows:
                target = dict(zip(["id","name","type","description","chat_id","preset_id","embedding_id","created_at","metadata"], target_rows[0]))
